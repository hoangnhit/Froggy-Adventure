    ...
    public override void FixedUpdate()
    {
        ...
        else
        {
            if (_playerStateManager.GetIsFacingRight())
                _playerStateManager.GetRigidBody2D().linearVelocity = new Vector2(_playerStateManager.GetPlayerStats.SpeedX, _playerStateManager.GetRigidBody2D().linearVelocity.y);
            else
                _playerStateManager.GetRigidBody2D().linearVelocity = new Vector2(-_playerStateManager.GetPlayerStats.SpeedX, _playerStateManager.GetRigidBody2D().linearVelocity.y);
            //Debug.Log("here");
        }
